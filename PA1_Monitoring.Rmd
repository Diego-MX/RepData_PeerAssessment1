...
Moreover, the interval where Mr. Anon took the most steps on average is ``r alongDay %$% interval[which.max(steps.avg)] %>% int2str``, which accounted to ``r max(alongDay$steps.avg)`` of them.  That whole period between, say `8:00` and `9:00` is one highly stepped hour of the days of Mr. Anon.  
